...
export default function NewsfeedCard({ post, user, refetchPosts, profile, pinnedPost }) {
      ...
      <CardActions className={classes.postHeader}>
        <div className={classes.postedBy}>
          {post?.postedBy?.profilePicture ? (
            <button className={classes.btn} aria-label={`visit the profile page of ${post.postedBy.firstName}`}>
              <CustomMessageIcon pictureIcon={post.postedBy.profilePicture} myProfileUsername={post.postedBy.userName} />
            </button>
          ) : (
